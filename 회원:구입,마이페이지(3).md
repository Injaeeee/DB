...
"구입할 매장을 선택하세요"밑의 콤보박스를 누르면 5개의 매장이 뜨는데(A,B,C,D,E 매장) 그 중 B매장을 선택하면 해당 매장에서 판매하는 모든 상품들이 상품상세 밑 그리드뷰에 모두 표시되며 상품종류 밑 리스트박스에 과자버튼을 누르면 해당 매장의 과자상품들만 아래처럼 표시된다 
